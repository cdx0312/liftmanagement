...
public class MyClientStarter {
	...
	// 客户端处理器
	private static ClientAioHandler tioClientHandler = new MyClientAioHandler();
	// 客户端监听器
	private static ClientAioListener aioListener = new MyClientAioListener();
	// 客户端上下文管理
	...
	public static void processCommand(String line) throws Exception {
		...
		if ("login".equalsIgnoreCase(command)) {
			...
			MyPacket reqPacket = new MyPacket();
			reqPacket.setType(Type.LOGIN_REQ);
			reqPacket.setBody(Json.toJson(loginReqBody).getBytes(MyPacket.CHARSET));
			...
			while (!Thread.currentThread().isInterrupted()) {
				...
				reqPacket.setBody(Json.toJson(liftInformationReqBody).getBytes(MyPacket.CHARSET));
				Tio.send(clientChannelContext, reqPacket);
				Thread.sleep(2000);
			}
			...
			packet.setBody(Json.toJson(liftInfoDetailReqBody).getBytes(MyPacket.CHARSET));
			...
			reqPacket.setBody(Json.toJson(joinGroupReqBody).getBytes(MyPacket.CHARSET));
			...
			reqPacket.setBody(Json.toJson(groupMsgReqBody).getBytes(MyPacket.CHARSET));
			...
			reqPacket.setBody(Json.toJson(p2pReqBody).getBytes(MyPacket.CHARSET));
